...
export class VisionService {
    ...
    try {
      ...
      if (grounding && photoInputs.length < 2) {
        return {
          ...
          confidence: 0.98,
          reason: '接地安装须分别上传箱内主PE连接和箱外机壳接地照片（至少 2 张不同视角），当前照片不完整。',
          provider: 'siliconflow',
        };
      }
      if (grounding && photoInputs.length >= 2) {
        const { uniqueCount } = await this.summarizePhotoUniqueness(photoInputs);
        if (uniqueCount < 2) {
          return {
            status: CheckResult.FAIL,
            confidence: 0.97,
            reason: '接地安装的箱内、箱外照片不能是同一张/同角度重复，请分别补拍箱内PE与箱外机壳接地。',
            provider: 'siliconflow',
          };
        }
      }
  ...
  private acSideHardRules() {
    return [
      '【交流侧安装检查·主PE判定】',
      '必须清晰看到【独立的】主PE导体：裸铜编织带或铜芯接地线，压接在标有“PE”的端子螺栓上。',
      '【严禁误判】L1/L2/L3 相线线鼻子附近露出的金属编织屏蔽层/铠装丝 ≠ 主PE铜编织带。那是相线屏蔽，不能当作PE已连接。',
      '【合格】在 PE 标签旁有单独一根较粗裸铜编织带/铜芯压在PE螺栓上；同框柜门细黄绿跳线可并存，不否决。',
      '【不合格】',
      '- PE 螺栓空着，或只有柜门细黄绿跳线；',
      '- 只看见相线（黄/绿/红标识）端子，右侧PE位无线或只有细线；',
      '- 把相线护套外的编织屏蔽当成主PE。',
      'doorBondingJumperOnly=true 仅当完全没有主PE铜编织带、只有柜门黄绿跳线。',
  ...
  private enforceAcSideResult(
    ...
    const text = `${parsed.reason || ''} ${raw}`;
    const phaseShieldAsPe =
      /(?:相线|L1|L2|L3).{0,12}(?:屏蔽|铠装|编织)/.test(text) ||
      /(?:屏蔽|铠装).{0,12}(?:当作|当成|误认).{0,8}PE/.test(text);
    const affirmsCopperPe =
      /铜编织|裸铜编织|铜辫/.test(text) &&
      /PE|接地端子|接地螺栓/.test(text) &&
      /压接|紧固|接入|已接/.test(text) &&
      !phaseShieldAsPe &&
      !/未见|空着|未压接|未接入|看不清|不确定|疑似|细.*屏蔽|仅柜门|只有黄绿|相线.*编织/.test(
        text,
      );
    const deniesMainPe =
      phaseShieldAsPe ||
      /未见.*(?:铜编织|铜芯)|PE端子为空|PE.*空着|仅柜门黄绿|只有黄绿跳线|细.*屏蔽|看不清.*压接|未压接|相线屏蔽/.test(
        text,
      );
  ...
  /** 交流侧不合格纠偏：清晰铜编织带压接PE即可翻成合格（同框柜门黄绿跳线不否决）。 */
  private async probeAcMainPeStrict(args: {
    ...
    const content: Array<Record<string, unknown>> = [
      {
        type: 'text',
        text: [
          '你只判断交流侧【主PE端子】是否合格，不要被相线干扰。',
          '判 true：在标有 PE 的端子旁，有【独立于 L1/L2/L3 的】较粗裸铜编织带/铜芯压接在PE螺栓上。',
          '同框柜门细黄绿跳线可忽略，不否决已压接的主PE。',
          '判 false（常见误判点）：',
          '1) PE螺栓空着；',
          '2) 只有柜门黄绿跳线，没有主PE铜编织带；',
          '3) 把 L1/L2/L3 相线线鼻子附近露出的金属编织屏蔽层/铠装丝当成主PE（这不是PE）；',
          '4) 画面里只有三根相线，未见独立接到PE端子的铜编织带。',
          'reason 须写清：是“PE端子铜编织带”还是“相线屏蔽编织”。',
    ...
    args.photoInputs.forEach((dataUrl, index) => {
      content.push({
        type: 'text',
        text: `【现场图 ${index + 1}】先找 PE 标签，再看该螺栓上是否有独立铜编织带（不是相线屏蔽）`,
      });
    ...
    try {
      ...
      if (
        /相线.*(?:屏蔽|铠装|编织)|(?:屏蔽|铠装).*(?:当作|当成)?PE|只有相线|未见.*PE|PE.*空着|只有黄绿/.test(
          reason,
        )
      ) {
        return false;
      ...
      return /铜编织|裸铜|铜芯|PE/.test(reason) && !/相线屏蔽|铠装丝/.test(reason);
    } catch {
      return false;
    }
  }

  /** 交流侧合格前复核：否决相线屏蔽冒充主PE、空PE、仅柜门跳线。 */
  private async auditAcMainPeDefect(args: {
    ...
    const content: Array<Record<string, unknown>> = [
      {
        type: 'text',
        text: [
          '你是交流侧主PE复核员。必须区分：主PE铜编织带 vs 相线屏蔽编织层。',
          'peConnected=true 仅当：PE标签旁有独立粗铜编织带/铜芯压在PE螺栓上。',
          'peConnected=false 若：PE空着；只有柜门黄绿跳线；或只有L1/L2/L3相线鼻子旁的屏蔽编织（不是主PE）。',
          'doorJumperOnly=true 仅当完全没有主PE、只有柜门黄绿跳线。',
          '只输出 JSON：{"peConnected":true|false,"doorJumperOnly":true|false,"phaseShieldMistaken":true|false,"reason":"一句话"}',
        ].join('\n'),
      },
    ];
    args.photoInputs.forEach((dataUrl, index) => {
      content.push({
        type: 'text',
        text: `【交流侧现场图 ${index + 1}】确认PE端子，勿把相线屏蔽当成PE`,
      });
      content.push({ type: 'image_url', image_url: { url: dataUrl } });
    });
    ...
    try {
      ...
      const match = raw.match(/\{[\s\S]*\}/);
      if (!match) return args.original;
      const obj = JSON.parse(match[0]) as {
        peConnected?: unknown;
        doorJumperOnly?: unknown;
        phaseShieldMistaken?: unknown;
        reason?: string;
      };
      ...
      const shieldMistake =
        obj.phaseShieldMistaken === true ||
        obj.phaseShieldMistaken === 'true' ||
        obj.phaseShieldMistaken === 1;
      const reason = String(obj.reason || raw);
      if (
        shieldMistake ||
        doorOnly ||
        !peOk ||
        /相线.*屏蔽|屏蔽.*冒充|PE.*空着|只有黄绿|未见.*铜编织/.test(reason)
      ) {
        return {
          status: CheckResult.FAIL,
          confidence: 0.95,
          reason:
            '现场不满足：主PE铜芯接地线/铜编织带实际压接到PE端子（相线屏蔽编织或柜门黄绿跳线不能替代）。',
        };
      }
      return {
        ...args.original,
  ...
  private enforceGroundingResult(
    ...
    try {
      ...
      const asBool = (value: unknown) => value === true || value === 'true' || value === 1;
      const photoTypes = Array.isArray(evidence?.photoTypes)
        ? evidence.photoTypes.map((item) => String(item))
        : [];
      ...
      const hasInternalType =
        photoTypes.includes('internal_main_pe') || internalChecks.length > 0;
      ...
      const hasExternalPhoto =
        asBool(evidence?.hasExternalGroundPhoto) || hasExternalType || affirmsExternal;

      // 轻量路径可能不返回 photoChecks：只要顶层双连接成立即可，不再因缺逐张字段整项否决
      if (checks.length > 0 && photoTypes.length > 0 && photoTypes.length < photoCount) {
        missing.push('每张现场照片的独立分类');
      }
      if (!hasInternalPhoto) {
      ...
      if (
        sampleCount > 0 &&
        !asBool(evidence?.matchesSampleViews) &&
        !(internalConnected || affirmsInternal) &&
        !(externalConnected || affirmsExternal)
      ) {
  ...
  /** 接地：轻量双点审核；无论首轮结果如何，合格前必须再过箱内主PE严格复核。 */
  private async auditGroundingConnections(args: {
    ...
    // 分别严格确认箱内主PE + 箱外机壳接地（并行）
    try {
      const reason = first?.reason || '';
      const firstInternalFail = /箱内主PE|铜芯线\/铜编织带实际压接/.test(reason);
      const firstExternalFail = /箱外黄绿|箱外机壳/.test(reason);
      // 首轮已合格或原因含糊时，两点都要复核，防止“一张不合格仍整项过”
      const needInternal =
        !first || first.status === CheckResult.PASS || firstInternalFail || !firstExternalFail;
      const needExternal =
        !first || first.status === CheckResult.PASS || firstExternalFail || !firstInternalFail;

      const [internalOk, externalOk] = await Promise.all([
        needInternal
          ? this.probeAcMainPeStrict({
              apiKey: args.apiKey,
              baseUrl: args.baseUrl,
              model: args.model,
              photoInputs:
                args.photoInputs.length >= 2
                  ? args.photoInputs.slice(0, -1)
                  : args.photoInputs,
            }).catch(() => false)
  ...
  /** 单点探针：箱内/箱外分开看对应视角，避免柜门黄绿跳线干扰箱外判定。 */
    ...
    const content: Array<Record<string, unknown>> = [
      {
        ...
        text: [
          ...
          '箱内合格：必须清晰看见较粗裸铜编织带/铜芯压在PE螺栓上；细银灰线或仅柜门黄绿跳线 = 不合格。',
          '柜门黄绿跳线不能替代主PE。箱内拿不准 → internalMainPeConnected=false。',
          '箱外合格：黄绿线或黄色线鼻子接到机壳/支架/横担/抱箍；侧视能看见线走向即可。',
    ...
    return Promise.all(
      inputs.map(async (input, index) => {
        try {
          ...
          // 竖拍箱外图：底部常有机壳/支架接地点；横拍箱内图：右侧常有PE端子
          const isPortrait = height >= width * 1.15;
          const extract = isPortrait
            ? {
